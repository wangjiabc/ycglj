...
var state=getQueryString("state");
 ...
 $.post('/ycglj/mobile/register/userinfoByopenId.do', {
	...
	if (obj.name == null)
		location.href = "../1/userSetting.html";
 });
 
 wx.ready(function () {

	 try{
			document.querySelector('#position').onclick =function(){
				 // 2. 分享接口
	         wx.getLocation({
	            success : function(res) {
	                 ...
	                 $.get("/ycglj/mobile/map/baiduSwitch.do",{ //微信地理位置坐标转换成百度地图坐标
	                	 ...
	                	 var lat=result[0].y;
	                	 var lng=result[0].x;
	                	 location.href="position/businessMap.html?latitude="+lat+"&longitude="+lng;
	                 });
		 ...
		 try{
			document.querySelector('#case').onclick =function(){
				wx.getLocation({
		               success : function(res) {
		                    // alert(JSON.stringify(res));
		                    var latitude = res.latitude; // 纬度，浮点数，范围为90 ~ -90
		                    ...
		                    $.get("/ycglj/mobile/map/baiduSwitch.do",{ //微信地理位置坐标转换成百度地图坐标
		                   	 ...
		                   	 var lat=result[0].y;
		                   	 var lng=result[0].x;
		                   	 location.href="case/case_cells.html?latitude="+lat+"&longitude="+lng;
		                    });
				...
				wx.getLocation({
	               success : function(res) {
	                    ...
	                    $.get("/ycglj/mobile/map/baiduSwitch.do",{ //微信地理位置坐标转换成百度地图坐标
	                   	 ...
	                   	 var lat=result[0].y;
	                   	 var lng=result[0].x;
	                   	 location.href="../assetAdmin/assetCheck/addCheckInfoList.html?latitude="+lat+"&longitude="+lng;
	                    });
				...
				wx.getLocation({
	               success : function(res) {
	                    ...
	                    $.get("/ycglj/mobile/map/baiduSwitch.do",{ //微信地理位置坐标转换成百度地图坐标
	                   	 ...
	                   	 var lat=result[0].y;
	                   	 var lng=result[0].x;
	                   	 location.href="../assetAdmin/assetCheck/assetCheckList.html?latitude="+lat+"&longitude="+lng;
	                    });
